...
public class PromotionSystem {
    ...
    public void run() {
        try {
            ...
            long price = total(day, order);
            benefit(day, order, price);
            sale();
            badge();
        } catch (Exception e) {
    ...
    public long total(int day, List order) {
    ...
    public void benefit (int day, List order, long price) {
        //  기능 4. 총 지불금액이 10000원 넘는지 확인 후 주문정보 저장
        reserve.set(day, price);

        // 기능 5. 사용자가 받을 혜택 금액을 각각 계산
        out.benefit();
        discount.benefit(day, order);

        // 기능 6. 총 주문금액에 따라 증정여부 판단 후 혜택 출력
        out.gift();
        discount.viewGift();

        // 기능 7. 사용자가 받을 총 혜택 금액을 계산
        out.totalBenefit();
        sale.totalBenefit();
    }
    public void sale() {
        //기능 8. 혜택 출력, 총 할인가 및 예상 결제금액 출력
        out.sale();
        sale.sale();
    }
    public void badge() {
